        ...
        for mention, entities in mention2entity.items():
            for entity in entities:
                ...
                if mention in context:
                    ...
                    mentions_data.append({'text_id': text_id, 'entity_id': entity_id, 'span_start': span_start, 'span_end': span_end, 'span': span})
...
def check_span(mentions_df, texts_df, index):
      mention_row = mentions_df.iloc[index]
      span_start, span_end, span = mention_row.span_start, mention_row.span_end, mention_row.span
      ...
      return span_correct, entity_id_correct
...
def main():
    ...
    for index in indices:
        span_correct, entity_id_correct = check_span(mentions_df, texts_df, index)
        print(f"Index: {index}")
